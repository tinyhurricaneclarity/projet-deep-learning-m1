# convnet_MS_HS.py
# ConvNet from scratch sur images MS + HS fusionnées (130 canaux)

# Objectif : entraîner un réseau de neurones convolutif (ConvNet) from scratch sur la fusion des images MS (5 canaux) et HS (125 canaux).
# Les images HS (32x32) sont redimensionnées en 64x64 avant la fusion.
...
def import_images(class_names, dico, tr_or_te="train"):
    """
    Charge et fusionne les images MS et HS pour chaque image.
    MS  : 5 canaux (tif) 64x64
    HS  : 125 canaux (tif) 32x32 -> redimensionné en 64x64
    Concaténation -> 130 canaux au total
    """
    images = []
    labels = []
    ...
    for (j, i) in indices:
        ...
        # Chargement HS (125 canaux, 32x32 -> 64x64)
        ...
        # Concaténation MS + HS -> 130 canaux
...
# 130 canaux : 5 MS + 125 HS
train_transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.RandomHorizontalFlip(),
    transforms.RandomVerticalFlip(),
    transforms.RandomRotation(15),
    transforms.Normalize(mean=[0.5] * 130, std=[0.5] * 130)
])
...
class ConvNet(nn.Module):
    """
    ConvNet from scratch adapté aux images MS+HS fusionnées 64x64.
    130 canaux en entrée (5 MS + 125 HS).
    3 blocs convolutifs + couches fully connected.
    """
    def __init__(self, num_classes=3):
        super(ConvNet, self).__init__()

        # Bloc 1 : 130 canaux en entrée
        self.conv1 = nn.Conv2d(130, 32, kernel_size=3, padding=1)
...
print(f"Shape d'une image : {Train['images'][0].shape}")  # doit être (64, 64, 130)
...
for epoch in range(1, num_epochs + 1):
    ...
    if val_loss < best_val_loss:
        ...
        epochs_sans_amelioration = 0
        torch.save(model.state_dict(), "saved_models/convnet_MS_HS_best.pth")
...
model.load_state_dict(torch.load("saved_models/convnet_MS_HS_best.pth"))
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Courbe de loss - ConvNet MS+HS")
plt.legend()
plt.savefig("loss_convnet_MS_HS.png")
print("Courbe de loss sauvegardée dans loss_convnet_MS_HS.png")
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("Courbe d'accuracy - ConvNet MS+HS")
plt.legend()
plt.savefig("accuracy_convnet_MS_HS.png")
print("Courbe d'accuracy sauvegardée dans accuracy_convnet_MS_HS.png")
