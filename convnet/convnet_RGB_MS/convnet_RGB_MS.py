# convnet_RGB_MS.py
# ConvNet from scratch sur images RGB + MS fusionnées (8 canaux)

# Objectif : entraîner un réseau de neurones convolutif (ConvNet) from scratch sur la fusion des images RGB (3 canaux) et MS (5 canaux) du dataset Kaggle.
# La concaténation des deux modalités donne 8 canaux en entrée.
...
def import_images(class_names, dico, tr_or_te="train"):
    """
    Charge et fusionne les images RGB et MS pour chaque image.
    RGB : 3 canaux (png), MS : 5 canaux (tif)
    Concaténation -> 8 canaux au total
    """
    images = []
    labels = []
...
class ConvNet(nn.Module):
    """
    ConvNet from scratch adapté aux images RGB+MS fusionnées 64x64.
    8 canaux en entrée (3 RGB + 5 MS).
    3 blocs convolutifs + couches fully connected.
    """
    def __init__(self, num_classes=3):
        super(ConvNet, self).__init__()

        # Bloc 1 : 8 canaux en entrée (3 RGB + 5 MS)
...
# Chemins vers les deux modalités
...
print(f"Shape d'une image : {Train['images'][0].shape}")  # doit être (64, 64, 8)
...
plt.tight_layout()
plt.savefig("histogrammes_complets_RGB_MS.png")
print("Histogrammes sauvegardés dans histogrammes_complets_RGB_MS.png")
...
for epoch in range(1, num_epochs + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), "saved_models/convnet_RGB_MS_best.pth")
...
model.load_state_dict(torch.load("saved_models/convnet_RGB_MS_best.pth"))
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Courbe de loss - ConvNet RGB+MS")
plt.legend()
plt.savefig("loss_convnet_RGB_MS.png")
print("Courbe de loss sauvegardée dans loss_convnet_RGB_MS.png")
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("Courbe d'accuracy - ConvNet RGB+MS")
plt.legend()
plt.savefig("accuracy_convnet_RGB_MS.png")
print("Courbe d'accuracy sauvegardée dans accuracy_convnet_RGB_MS.png")
