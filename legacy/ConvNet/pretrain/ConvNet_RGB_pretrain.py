...
# Accès au module parent pour importer model.py et config.py
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from model import ConvNet
from config import SEED_RANDOM, SEED_TORCH, BATCH_SIZE
...
WHEAT_PATH   = "/home/mona/wheat_leaf"
...
print("Chargement des images Wheat Leaf.")
images, labels = import_images_wheat(WHEAT_PATH)
...
# ConvNet importé depuis model.py — même architecture que le reste du projet
...
for epoch in range(1, num_epochs + 1):
    ...
    if val_loss < best_val_loss:
        ...
        epochs_sans_amelioration = 0
        torch.save(model.state_dict(), f"{SAVE_DIR}/convnet_wheat_pretrained.pth")
...
model.load_state_dict(torch.load(f"{SAVE_DIR}/convnet_wheat_pretrained.pth"))
...
plt.legend()
plt.savefig(f"{SAVE_DIR}/loss_pretrain_wheat.png")
plt.close()
...
plt.legend()
plt.savefig(f"{SAVE_DIR}/accuracy_pretrain_wheat.png")
plt.close()

print(f"\nPré-entraînement terminé. Modèle sauvegardé dans {SAVE_DIR}/")
