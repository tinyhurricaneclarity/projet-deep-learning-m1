...
def import_images(class_names, dico, tr_or_te="train"): #pour HS
    images = []
    labels = []
    ...
    for (j, i) in indices:
        ...
        # Force 125 canaux : certaines images HS ont 126 canaux; le dernier étant utile on garde seulement 125 canaux.
        if img.shape[2] > 125:
            img = img[:, :, :125]
...
train_transform = transforms.Compose([
    ...
    transforms.Normalize(mean=[0.5] * 125, std=[0.5] * 125) #permet de "lisser" la distribution des tensors (valeurs autour de 0) pour accélérer l’entraînement et rendre l’optimisation plus stable
])
